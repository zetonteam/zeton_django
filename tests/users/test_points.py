...
from users.models import Caregiver, Point, Student
...
def test_add_point_invalid_json(client):
    ...
    response = client.post("/api/users/points/", {}, content_type="application/json")
...
def test_get_single_point_incorrect_id(client):
    response = client.get("/api/users/students/2/points/foo")
...
def test_get_points_for_filtered_student(client):
    ...
    Point.objects.create(value=22, assigner=caregiver, student=requested_student)
    Point.objects.create(value=17, assigner=caregiver, student=requested_student)
    Point.objects.create(value=144, assigner=caregiver, student=other_student)
...
def test_get_points_for_filtered_student_invalid_query_string(client):
    ...
    response = client.get(
        f"/api/users/points/?invalidQueryString={requested_student.pk}"
    )
